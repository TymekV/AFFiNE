...
AFFiNE.server.host = 'affine.tymek.ovh';
...
AFFiNE.use('oauth', {
  providers: {
    ...
    oidc: {
      ...
      args: {
        scope: process.env.OAUTH_OIDC_SCOPE ?? 'openid email profile',
        claim_id: process.env.OAUTH_OIDC_CLAIM_MAP_USERNAME ?? 'preferred_username',
        claim_email: process.env.OAUTH_OIDC_CLAIM_MAP_EMAIL ?? 'email',
        claim_name: process.env.OAUTH_OIDC_CLAIM_MAP_USERNAME ?? 'name',
      },
    },
  },
});
